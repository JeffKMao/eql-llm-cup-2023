...
def load_model_tokenizer_for_generate(
    ...
    """
    tokenizer = LlamaTokenizer.from_pretrained(pretrained_model_name_or_path, padding_side="left")
    model = AutoModelForCausalLM.from_pretrained(
        pretrained_model_name_or_path, torch_dtype=torch.bfloat16, trust_remote_code=True#, device_map="auto"
    )
...
class InstructionTextGenerationPipeline(Pipeline):
    def __init__(
        self, *args, do_sample: bool = True, max_new_tokens: int = 2048, top_p: float = 0.92, top_k: int = 0, **kwargs
    ):
    ...
    def _sanitize_parameters(self,
        ...
        postprocess_params["return_full_text"] = False
    ...
    def postprocess(self, model_outputs, response_key_token_id, end_key_token_id, return_full_text: bool = False):
        ...
        for sequence in generated_sequence:
            ...
            if not decoded:
                ...
                m = re.search(r"#+\s*Response:\s*(.+?)</s>", fully_decoded, flags=re.DOTALL)
                #m = re.search(r"[/INST]\s*(.+?)</s>", fully_decoded, flags=re.DOTALL)
                if m:
                    ...
                    #m = re.search(r"[/INST]\s*(.+)", fully_decoded, flags=re.DOTALL)
                    if m:
                        decoded = m.group(1).strip()
